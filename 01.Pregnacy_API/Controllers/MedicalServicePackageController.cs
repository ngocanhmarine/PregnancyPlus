	...
	public class MedicalServicePackageController : ApiController
	{
		...
		public HttpResponseMessage Get([FromUri]preg_medical_service_package data)
		{
			try
			{
				...
				if (!data.DeepEquals(new preg_medical_service_package()))
				{
		...
		public HttpResponseMessage Get(string id)
		{
			try
			{
				preg_medical_service_package data = dao.GetItemByID(Convert.ToInt32(id)).FirstOrDefault();
				if (data != null)
				{
		...
		public HttpResponseMessage Delete(string id)
		{
			try
			{
				preg_medical_service_package item = dao.GetItemByID(Convert.ToInt32(id)).FirstOrDefault();
				if (item == null)
				{
		...
		public HttpResponseMessage UpdateData(string id, preg_medical_service_package dataUpdate)
		{
			try
			{
				if (!dataUpdate.DeepEquals(new preg_medical_service_package()))
				{
